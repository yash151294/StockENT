...
const ProductDetailPage: React.FC = () => {
  ...
  const handleContactSeller = async () => {
    ...
    router.push(`/messages?product=${productData.id}&seller=${productData.seller.id}`);
  };
                ...
                <Box display="flex" flexDirection="column" gap={2}>
                  ...
                  {(productData.listingType || 'FIXED_PRICE') === 'AUCTION' && productData.auction && (
                    <Button
                      ...
                      size="large"
                      startIcon={<Gavel />}
                      onClick={() => router.push(`/auctions/${productData.auction.id}`)}
                      fullWidth
                    >
                      View Auction
                    </Button>
                  ...
                  {(productData.listingType || 'FIXED_PRICE') === 'FIXED_PRICE' && state.user?.role === 'BUYER' && (
                    <Box>
                      <Box sx={{ mb: 2 }}>
                      ...
                      <Button
                        ...
                        disabled={isInCart(productData.id, 'DIRECT')}
                      >
                        {isInCart(productData.id, 'DIRECT') ? 'Already in Cart' : 'Add to Cart'}
                      </Button>
                    </Box>
                  )}
